...
* @Last Modified by:   dm
* @Last Modified time: 2016-01-10 01:40:51
*/
...
function creteRoom(userNames) {
    let u = baseUrl + '/webwxcreatechatroom'
    ...
    return new Promise((resolve, reject) => {
        request(serialize(u, params), data, headers).then((data) => {
            ...
            _.each(list, (member) => {
                if(member.MemberStatus == 4) _deletedList.push(member)
            })

            if(ret == 0) resolve(userNames)
            else reject(errMsg || '建群失败')
        }, reject)
    })
}

function add2Room(userNames) {
    let u = baseUrl + '/webwxupdatechatroom'
    ...
    return new Promise((resolve, reject) => {
        request(serialize(u, params), data, headers).then((data) => {
            ...
            if(ret == 0) resolve(userNames)
            else reject(errMsg || '加群失败')
        }, reject)
    })
}

function removeFromRoom(userNames) {
    let u = baseUrl + '/webwxupdatechatroom'
    ...
    return request(serialize(u, params), data, headers).then((data) => {
        data = JSON.parse(data.toString())

        let ret = data['BaseResponse']['Ret']
        let errMsg = data['BaseResponse']['ErrMsg']

        if(ret != 0) printErr(errMsg || '移除群失败')
        ...
        .then((list) => {
            ...
            let check = () => {
                if(group == groupNum) {
                    ...
                    console.log('查询完毕，被【%d】个好友删除'.cyan, _deletedList.length)
                    console.log('==========删除你的好友============')
                ...
                let users = list.slice((group - 1)*MAX_GROUP_NUM, MAX_GROUP_NUM)
                let userNames = _.map(users, (v) => {
                    usersMap[v.UserName] = v
                    return v.UserName
                })
                let createOrAdd = _roomName ? creteRoom : add2Room

                createOrAdd(userNames)
                    .then(removeFromRoom, printErr)
                    .then((done) => {
                        if(done) setTimeout(check, SEARCH_INTERVAL)
                    })
            }
