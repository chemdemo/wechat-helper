...
* @Last Modified by:   dm
* @Last Modified time: 2016-01-14 23:22:32
*/
...
let request = (u, data, headers) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        req.setTimeout(3000, () => reject(`request ${u} timeout`))
...
let creteRoom = (userNames) => {
    ...
    let memberList = userNames.map((nm) => {
        return {'UserName': nm}
    })
...
let checkWhoDeletedMe = (members) => {
    ...
    let check = () => {
        if(group == groupNum) {
            ...
            _deletedList = _.map(_deletedList, (v) => usersMap[v]['NickName'])
